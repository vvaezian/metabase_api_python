...
class Metabase_API_Test(unittest.TestCase):
    ...
    def tearDown(self):  # runs after every test
        for item_type, item_id_list in Metabase_API_Test.cleanup_objects.items():
            ...
            if item_type in [
                "collection",
                "segment",
            ]:  # these cannot be deleted, so we archive them
                for item_id in item_id_list:
    ...
    def test_get_columns_name_id(self):
        name_id_mapping = mb.get_columns_name_id(
            table_id=1
        )  # table with id 1 is the products table from sample dataset
    ...
    def test_create_card(self):
        ...
        res1 = mb.create_card(
            card_name=f"test_create_card_{t}",
        ...
        self.assertEqual(
            str(error.exception), "Either the name or id of the table must be provided."
        )

        # check to make sure the cards were created in the right collection
        collection_IDs_of_created_cards = {
            i["collection_id"] for i in [res1, res2, res3]
        }
        ...
        Metabase_API_Test.cleanup_objects["card"].extend(
            [res1["id"], res3["id"], res3["id"]]
        )

    def test_create_collection(self):
        t = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        res = mb.create_collection(
            f"test_create_collection {t}", parent_collection_id=2, return_results=True
        )
    ...
    def test_copy_card(self):
        t = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        newCard_id = mb.copy_card(
            source_card_id=1,
            destination_collection_id=1,
            destination_card_name="test_copy_card_{}".format(t),
        )
    ...
    def test_copy_dashboard(self):
        ...
        # shallow copy
        dup_dashboard_id_shallow = mb.copy_dashboard(
            source_dashboard_id=1,
            destination_collection_id=1,
            postfix="_dup_shallow_{}".format(t),
        )

        # deep copy
        dup_dashboard_id_deep = mb.copy_dashboard(
            source_dashboard_id=1,
            destination_collection_id=1,
        ...
        # add to cleanup list
        Metabase_API_Test.cleanup_objects["dashboard"].extend(
            [dup_dashboard_id_shallow, dup_dashboard_id_deep]
        )
    ...
    def test_copy_collection(self):
        ...
        mb.copy_collection(
            ...
            destination_parent_collection_id=1,
    ...
    def test_get_card_data(self):
        ...
        # filtered data
        res = mb.get_card_data(
            card_id=2,
            parameters=[
                {
                    "type": "string/=",
                    "value": ["row1 cell1", "row3 cell1"],
    ...
    def test_clone_card(self):
        ...
        res = mb.clone_card(
            ...
            9,
            10,
            new_card_name="test_clone_native",
            new_card_collection_id=1,
            return_card=True,
        )
        ...
        res2 = mb.clone_card(
            ...
            9,
            10,
            new_card_name="test_clone_simple1",
            new_card_collection_id=1,
            return_card=True,
        )
        res3 = mb.clone_card(
            ...
            9,
            10,
            new_card_name="test_clone_simple2",
            new_card_collection_id=1,
            return_card=True,
        )
        ...
        self.assertTrue(
            res3["dataset_query"] == expected_res3_query_version1
            or res3["dataset_query"] == expected_res3_query_version2,
            res3["dataset_query"],
        )

        # add to cleanup list
        Metabase_API_Test.cleanup_objects["card"].extend(
            [res["id"], res2["id"], res3["id"]]
        )
